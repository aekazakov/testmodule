...
class testmodule(object):
    ...
    def count_contigs(self, workspace_name, contigset_id, json_rpc_context = None):
        ...
        resp = self._call('testmodule.count_contigs',
                          [workspace_name, contigset_id], json_rpc_context)
        return resp[0]
  
    def get_string(self, workspace_name, user_name, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method get_string: argument json_rpc_context is not type dict as required.')
        resp = self._call('testmodule.get_string',
